...
import { loadEnvConfig, resolveAgentCommand } from "./environment.js";
...
describe("loadEnvConfig", () => {
  it("returns defaults when env is empty", () => {
    ...
    expect(loaded.acpPoolSize).toBe(0);
    expect(loaded.acpPoolMaxRequests).toBe(100);
    expect(loaded.acpPoolMaxAgeMs).toBe(3_600_000);
  ...
  it("parses booleans, numbers, and model normalization", () => {
    const loaded = loadEnvConfig({
      env: {
        ASTERMUX_FORCE: "yes",
        ASTERMUX_APPROVE_MCPS: "on",
        ASTERMUX_STRICT_MODEL: "off",
  ...
  it("parses ASTERMUX_MODE and marks chat-only env as explicit", () => {
    const loaded = loadEnvConfig({
      env: {
        ASTERMUX_MODE: "plan",
  ...
  it("throws on invalid ASTERMUX_MODE", () => {
    expect(() =>
      loadEnvConfig({ env: { ASTERMUX_MODE: "nope" }, cwd: "/w" }),
  ...
  it("parses guarded DSH auto-mode settings", () => {
    const loaded = loadEnvConfig({
      env: {
        ASTERMUX_DSH_AUTO_MODE: "true",
        ASTERMUX_DSH_SYSTEM_MARKER: "custom-dsh",
        ASTERMUX_DSH_PLAN_MARKER: "custom-plan",
      },
      cwd: "/w",
    });
  ...
  it("resolves workspace and explicit paths from cwd", () => {
    const loaded = loadEnvConfig({
      env: {
        ASTERMUX_WORKSPACE: "./repo",
        ASTERMUX_SESSIONS_LOG: "./logs/sessions.log",
        ASTERMUX_TLS_CERT: "./certs/dev.crt",
        ASTERMUX_TLS_KEY: "./certs/dev.key",
      },
      cwd: "/tmp/project",
    });
  ...
  it("uses HOME before USERPROFILE for default sessions log path", () => {
    ...
    expect(loaded.sessionsLogPath).toBe(
      path.join("/home/alice", ".astermux", "sessions.log"),
    );
  });
  ...
  it("uses USERPROFILE when HOME is not set", () => {
    ...
    expect(loaded.sessionsLogPath).toBe(
      path.join("C:\\Users\\alice", ".astermux", "sessions.log"),
    );
  });
  ...
  it("applies tailscale host fallback only when host is unset", () => {
    ...
    expect(
      loadEnvConfig({
        env: { ASTERMUX_HOST: "10.0.0.5" },
        tailscale: true,
      }).host,
    ).toBe("10.0.0.5");
  });

  it("parses ASTERMUX_ACCOUNT_DIRS as comma-separated absolute paths", () => {
    const loaded = loadEnvConfig({
      env: { ASTERMUX_ACCOUNT_DIRS: "/acc/a,/acc/b,/acc/c" },
      cwd: "/workspace",
    });
  ...
  it("trims whitespace from each dir in ASTERMUX_ACCOUNT_DIRS", () => {
    const loaded = loadEnvConfig({
      env: { ASTERMUX_ACCOUNT_DIRS: " /acc/a , /acc/b " },
      cwd: "/workspace",
    });
    expect(loaded.configDirs).toEqual([
      path.resolve("/workspace", "/acc/a"),
  ...
  it("resolves relative dirs in ASTERMUX_ACCOUNT_DIRS against cwd", () => {
    const loaded = loadEnvConfig({
      env: { ASTERMUX_ACCOUNT_DIRS: "./accounts/a,./accounts/b" },
      cwd: "/workspace",
    });
    expect(loaded.configDirs).toEqual([
  ...
  it("returns empty configDirs when ASTERMUX_ACCOUNT_DIRS is unset and no accounts dir", () => {
    const loaded = loadEnvConfig({
  ...
  it("multiPort is parsed from ASTERMUX_MULTI_PORT env var", () => {
    expect(
      loadEnvConfig({ env: { ASTERMUX_MULTI_PORT: "true" } }).multiPort,
    ).toBe(true);
    expect(
      loadEnvConfig({ env: { ASTERMUX_MULTI_PORT: "1" } }).multiPort,
    ).toBe(true);
    expect(
      loadEnvConfig({ env: { ASTERMUX_MULTI_PORT: "false" } }).multiPort,
    ).toBe(false);
  });
  ...
  it("maxMode is parsed from ASTERMUX_MAX_MODE", () => {
    expect(
      loadEnvConfig({ env: { ASTERMUX_MAX_MODE: "true" } }).maxMode,
    ).toBe(true);
  });
  ...
  it("winCmdlineMax is parsed from ASTERMUX_WIN_CMDLINE_MAX and clamped", () => {
    expect(
      loadEnvConfig({ env: { ASTERMUX_WIN_CMDLINE_MAX: "25000" } })
        .winCmdlineMax,
    ).toBe(25_000);
    expect(
      loadEnvConfig({ env: { ASTERMUX_WIN_CMDLINE_MAX: "999999" } })
        .winCmdlineMax,
    ).toBe(32_700);
    expect(
      loadEnvConfig({ env: { ASTERMUX_WIN_CMDLINE_MAX: "100" } })
        .winCmdlineMax,
    ).toBe(4096);
  });

  it("parses ASTERMUX_CONTEXT_PREAMBLE", () => {
    expect(loadEnvConfig({ env: {} }).contextPreamble).toBe(true);
    expect(
      loadEnvConfig({ env: { ASTERMUX_CONTEXT_PREAMBLE: "false" } })
        .contextPreamble,
    ).toBe(false);
  });
  ...
  it("loads ASTERMUX_CONTEXT_EXTRA", () => {
    ...
    expect(
      loadEnvConfig({
        env: { ASTERMUX_CONTEXT_EXTRA: "note" },
      }).contextExtra,
    ).toBe("note");
  });

  it("truncates ASTERMUX_CONTEXT_EXTRA", () => {
    const long = "a".repeat(2500);
    const extra = loadEnvConfig({
      env: { ASTERMUX_CONTEXT_EXTRA: long },
    }).contextExtra;
    expect(extra!.length).toBe(400);
    expect(extra!.endsWith("…")).toBe(true);
  });

  it("parses ACP worker pool settings with bounds", () => {
    const loaded = loadEnvConfig({
      env: {
        ASTERMUX_ACP_POOL_SIZE: "3",
        ASTERMUX_ACP_INTERACTIVE_WARM_SIZE: "1",
        ASTERMUX_ACP_BATCH_WARM_SIZE: "2",
        ASTERMUX_ACP_TOOL_WARM_SIZE: "1",
        ASTERMUX_ACP_ELASTIC_IDLE_MS: "45000",
        ASTERMUX_ACP_POOL_MAX_REQUESTS: "75",
        ASTERMUX_ACP_POOL_MAX_AGE_MS: "1800000",
      },
    });
    expect(loaded.acpPoolSize).toBe(3);
    expect(loaded.acpInteractiveWarmSize).toBe(1);
    expect(loaded.acpBatchWarmSize).toBe(2);
    expect(loaded.acpToolWarmSize).toBe(1);
    expect(loaded.acpElasticIdleMs).toBe(45_000);
    expect(loaded.acpPoolMaxRequests).toBe(75);
    expect(loaded.acpPoolMaxAgeMs).toBe(1_800_000);
    expect(
      loadEnvConfig({ env: { ASTERMUX_ACP_POOL_SIZE: "99" } }).acpPoolSize,
    ).toBe(8);
  });

  it("parses ASTERMUX_PROMPT_VIA_STDIN and ASTERMUX_USE_ACP", () => {
    expect(
      loadEnvConfig({ env: { ASTERMUX_PROMPT_VIA_STDIN: "true" } })
        .promptViaStdin,
    ).toBe(true);
    expect(loadEnvConfig({ env: { ASTERMUX_USE_ACP: "1" } }).useAcp).toBe(
      true,
    );
  });
});
...
describe("discoverAccountDirs filtering", () => {
  ...
  function makeTmpAccounts(): string {
    ...
    const accountsDir = path.join(tmpBase, ".astermux", "accounts");
  ...
  it("ASTERMUX_ACCOUNT_DIRS takes priority over auto-discovery", () => {
    const accountsDir = makeTmpAccounts();
    writeCliConfig(path.join(accountsDir, "discovered"), true);

    const loaded = loadEnvConfig({
      env: { HOME: tmpBase, ASTERMUX_ACCOUNT_DIRS: "/explicit/dir" },
      cwd: "/workspace",
    });
    expect(loaded.configDirs).toEqual([
