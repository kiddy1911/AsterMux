...
describe("buildPromptFromMessages", () => {
  ...
  it("preserves assistant tool calls and non-trailing tool results", () => {
    const messages = [
      { role: "user", content: "Search for the version" },
      {
        role: "assistant",
        ...
        tool_calls: [
          {
            id: "call_search_1",
            type: "function",
            function: { name: "WebSearch", arguments: '{"query":"cliproxyapi"}' },
          },
        ],
      },
      {
    ...
    expect(prompt).toContain(
      'Assistant tool call [id=call_search_1 name=WebSearch]: {"query":"cliproxyapi"}',
    );
    expect(prompt).toContain("Tool result [call_id=call_search_1]: v1.2.3");
    expect(prompt).toContain("User: <system-reminder>continue</system-reminder>");
  });
